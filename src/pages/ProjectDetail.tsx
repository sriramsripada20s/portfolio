...
export default function ProjectDetail() {
  ...
  return (
    <div className="min-h-screen pt-24 pb-24 px-6 md:px-12 max-w-6xl mx-auto">
      ...
      <Link
        to="/"
        className="inline-flex items-center gap-2 text-[12px] font-medium text-[#6b7a96] hover:text-white transition-colors mb-10 tracking-wide"
      >
        <ArrowLeft size={14} />
        All Projects
      </Link>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {project.domain.map(d => (
          <span
            key={d}
            className="text-[11px] font-semibold px-3 py-1 rounded-md bg-sky-950/60 text-sky-400 border border-sky-800/30 tracking-wide uppercase"
          >
            {d}
          </span>
        ))}
        ...
        <span className="text-[12px] text-[#6b7a96] self-center ml-2">
          {project.year}
        </span>
      </div>
      ...
      <p className="text-[15px] text-sky-400 font-medium mb-8">
        {project.subtitle}
      </p>
        ...
        {project.metrics.map((m, i) => {
          ...
          return (
            <div key={i} className={`${c.bg} border ${c.border} rounded-2xl p-5`}>
              <div className={`text-[28px] font-bold leading-none mb-1 ${c.text}`}>
                {m.value}
              </div>
              <div className="text-[11px] text-[#6b7a96] font-medium uppercase tracking-wider">
                {m.label}
              </div>
            </div>
          );
      ...
      <div className="grid md:grid-cols-3 gap-10">
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="bg-[#161b27] border border-[#232d3f] rounded-2xl p-6">
            <h3 className="text-[11px] font-semibold text-sky-400 tracking-widest uppercase mb-4">
              Key Results
            </h3>
              ...
              {project.metrics.map((m, i) => {
                ...
                return (
                  <div key={i} className="flex items-center gap-3">
                    <div className={`text-[18px] font-bold min-w-[60px] ${c.text}`}>
                      {m.value}
                    </div>
                    <div className="text-[12px] text-[#6b7a96]">
                      {m.label}
                    </div>
                  </div>
                );
          ...
          <div className="bg-[#161b27] border border-[#232d3f] rounded-2xl p-6">
            ...
            <div className="flex flex-wrap gap-2">
              {project.tech.map(t => (
                <span
                  key={t}
                  className="text-[11px] px-3 py-1.5 rounded-lg border border-[#232d3f] text-[#8892a4] bg-[#0f1117]"
                >
                  {t}
                </span>
              ))}
          ...
          <div className="bg-[#161b27] border border-[#232d3f] rounded-2xl p-6">
            ...
            {project.githubUrl ? (
              ...
              <p className="text-[13px] text-[#6b7a96]">
                Internal production project — no public repo.
              </p>
            )}
          </div>
          ...
          <div className="bg-[#161b27] border border-[#232d3f] rounded-2xl p-6">
            ...
            <div className="flex flex-wrap gap-2">
              {project.domain.map(d => (
                <span
                  key={d}
                  className="text-[12px] px-3 py-1.5 rounded-lg bg-sky-950/60 text-sky-400 border border-sky-800/30"
                >
                  {d}
                </span>
              ))}
      ...
      {others.length > 0 && (
        <div className="mt-20">
          <div className="flex items-center justify-between mb-6">
            ...
            <Link to="/" className="text-[13px] text-sky-400 hover:underline">
              View all →
            </Link>
          </div>
                ...
                <div className="p-5">
                  <div className="flex flex-wrap gap-1.5 mb-2">
                    {p.domain.slice(0, 1).map(d => (
                      <span
                        key={d}
                        className="text-[10px] font-semibold px-2 py-0.5 rounded bg-sky-950/60 text-sky-400 border border-sky-800/30 uppercase tracking-wide"
                      >
                        {d}
                      </span>
                    ))}
                  ...
                  <p className="text-[12px] text-[#6b7a96] leading-relaxed line-clamp-2">
                    {p.description}
                  </p>
                </div>
              </Link>
